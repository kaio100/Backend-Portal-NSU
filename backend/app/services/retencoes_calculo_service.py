...
def calcular_retencoes_esperadas(dados: dict[str, Any], regra: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not regra:
        alertas.append("Subitem LC116 nao identificado no XML.")
    ...
    status_iss = "Informado" if parse_decimal_xml(dados.get("valor_iss")) is not None else "Nao informado"
    ...
    liquido_calculado = money(valor_servico - valor_irrf - valor_inss - valor_pis - valor_cofins - valor_csll - (valor_iss_retido if iss_retido else 0) - outras - desc_inc - desc_cond)
